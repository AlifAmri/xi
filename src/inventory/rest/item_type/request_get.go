...
func Show(id int64) (*model.ItemType, error) {
	m := new(model.ItemType)
...
func Get(rq *orm.RequestQuery) (m *[]model.ItemType, total int64, err error) {
	// make new orm query
	q, _ := rq.Query(new(model.ItemType))
