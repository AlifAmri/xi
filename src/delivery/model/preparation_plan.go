...
func init() {
	orm.RegisterModel(new(PreparationPlan))
}

// PreparationPlan model for PreparationPlan table.
type PreparationPlan struct {
	...
	Items []*PreparationPlanItem `orm:"reverse(many)" json:"items,omitempty"`
}

// MarshalJSON customized data struct when marshaling data
// into JSON format, all Primary key & Foreign key will be encrypted.
func (m *PreparationPlan) MarshalJSON() ([]byte, error) {
	type Alias PreparationPlan
...
// Save inserting or updating PreparationPlan struct into PreparationPlan table.
// It will updating if this struct has valid Id
// if not, will inserting a new row to PreparationPlan.
// The field parameter is an field that will be saved, it is
// usefull for partial updating data.
func (m *PreparationPlan) Save(fields ...string) (err error) {
	o := orm.NewOrm()
	if m.ID > 0 {
...
// Delete permanently deleting PreparationPlan data
// this also will truncated all data from all table
// that have relation with this PreparationPlan.
func (m *PreparationPlan) Delete() (err error) {
	o := orm.NewOrm()
	if m.ID > 0 {
		var i int64
...
// assigned.
func (m *PreparationPlan) Read(fields ...string) error {
	o := orm.NewOrm()
