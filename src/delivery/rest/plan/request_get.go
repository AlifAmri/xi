...
func Show(id int64) (*model.PreparationPlan, error) {
	m := new(model.PreparationPlan)
	o := orm.NewOrm()
	if err := o.QueryTable(m).Filter("id", id).RelatedSel().Limit(1).One(m); err != nil {
		return nil, err
	}

	o.LoadRelated(m, "Items")
	return m, nil
}
...
func Get(rq *orm.RequestQuery) (m *[]model.PreparationPlan, total int64, err error) {
	// make new orm query
	q, _ := rq.Query(new(model.PreparationPlan))
