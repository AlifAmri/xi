...
func Show(id int64) (*model.Receiving, error) {
	m := new(model.Receiving)
	o := orm.NewOrm()
...
func Get(rq *orm.RequestQuery) (m *[]model.Receiving, total int64, err error) {
	// make new orm query
	q, _ := rq.Query(new(model.Receiving))
