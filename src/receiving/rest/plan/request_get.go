...
func Show(id int64) (*model.ReceiptPlan, error) {
	m := new(model.ReceiptPlan)
	o := orm.NewOrm()
...
func Get(rq *orm.RequestQuery) (m *[]model.ReceiptPlan, total int64, err error) {
	// make new orm query
	q, _ := rq.Query(new(model.ReceiptPlan))
